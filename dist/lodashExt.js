 ...
 */
function addItems(source, items) {
 ...
 */
function clearDefaults(obj, defaults) {
...
function truncFactory() {
     ...
     * @returns {string}
     */
    return function (input, limit, byWord) {
 ...
 */
function getCharFromKeypress(event) {
    if (event.which === null) {
        // спец. символ
        if (event.keyCode < 32) {
            return null;
 ...
 * @returns {boolean}
 */
function hasValues(obj, _objects) {
...
function inherit(Child, Parent, overrides) {
    var F = (function () {
        function F() {
        }
        return F;
 ...
 */
function renameFields(obj, hash) {
    if (!_.isObject(obj)) {
        return obj;
    }
    _.forEach(hash, function (newFieldName, oldFieldName) {
 ...
 */
function waitFor(config, context) {
